...
export default function AdminProductEdit() {
  ...
  const [form, setForm] = useState({ name: "", slug: "", brief: "", description: "", categoryId: "", brand: "", isFeatured: false })
  ...
  useEffect(() => {
    Promise.all([
      ...
      setForm({ name: p.name, slug: p.slug, brief: p.brief || "", description: p.description, categoryId: p.categoryId, brand: p.brand || "", isFeatured: p.isFeatured })
  ...
  const updateVariant = (i: number, field: string, value: any) => {
    const updated = [...variants]; (updated[i] as any)[field] = value; setVariants(updated)
  }
  const removeVariant = (i: number) => {
    if (variants.length <= 1) return; setVariants(variants.filter((_, idx) => idx !== i))
  }
  ...
  return (
    <div>
      <h1 className="font-serif text-3xl text-[#062437] mb-8">Edit Product</h1>
          ...
          <div className="bg-white border border-[#e3e2e4] p-6">
            ...
            <div className="space-y-4">
              ...
              <input type="text" placeholder="Slug" required value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value })}
                className="w-full border border-[#e3e2e4] px-4 py-3 text-sm focus:outline-none focus:border-[#062437]" />
              <input type="text" placeholder="Brief" value={form.brief} onChange={(e) => setForm({ ...form, brief: e.target.value })}
                className="w-full border border-[#e3e2e4] px-4 py-3 text-sm focus:outline-none focus:border-[#062437]" />
              ...
              <label className="flex items-center gap-3 text-sm">
                <input type="checkbox" checked={form.isFeatured} onChange={(e) => setForm({ ...form, isFeatured: e.target.checked })} className="accent-[#062437]" />
                Featured Product
              </label>
            </div>
          </div>
